...
JNIEXPORT void JNICALL Java_com_example_yt_reforceapk_ReinforceApplication_decrypt
        (JNIEnv *env, jobject jobj, jstring src, jstring dist, jint mode) {

    const char * source_path = env ->GetStringUTFChars(src,JNI_FALSE);
    const char * des_path = env ->GetStringUTFChars(dist,JNI_FALSE);
    const char *password = "0f1571c947d9e8570cb7ade6af0f6793";
    if (source_path == NULL || des_path == NULL){
        return;//抛出OOM异常
    }

    aes(source_path,des_path,password,mode);
    env ->ReleaseStringUTFChars(src,source_path);
    env ->ReleaseStringUTFChars(dist,des_path);
...
//        int method = 0;//1表示加密， 0表示解密
    //待加密/解密文件存放路径
//        const char * source_path = "/Users/yt/Desktop/cp/test2.txt";
    // 加密/解密后文件存放路径
//        const char * des_path = "/Users/yt/Desktop/cp/test3.txt";
    // 32位16进制密钥
//        const char * password = "0f1571c947d9e8590cb7add6af7f6798";
//        aes(source_path, des_path, password, method);
//        printf("success!!!!!!!!!!");
//}

void aes(const char* source_path,const char* des_path,const char* password, int method){
    //将密钥转换成4*4数组
    int p[4][4];
    for (int m = 0; m < 4; ++m) {
        for (int i = 0; i < 4; ++i) {
